...
/**
 * A Dialog that slides in from an edge. Used for the mobile navigation drawer,
 * where it buys focus trapping, scroll locking and Escape-to-close for free.
 */
...
>(({ side = "left", className, children, hideClose, ...props }, ref) => (
  <SheetPrimitive.Portal>
    ...
    >
      {children}
      {!hideClose && (
        <SheetPrimitive.Close className="absolute right-4 top-4 rounded-lg p-1 text-muted-foreground opacity-70 transition-opacity hover:bg-secondary hover:opacity-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-ring">
          <X className="size-4" />
